function fetchSpecificEmails() {
  ...
  try {
    ...
    console.error("DockpitFree セクションでエラー:", e && e.stack ? e.stack : e);
  }
  ...
  try {
    ...
    if (whitepaperSheet) {
      ...
      whitepaperThreads.forEach(thread => {
        try {
          ...
          if (!match || !match[1]) {
            console.warn(`ホワイトペーパー件名にマッチしませんでした。スレッドをスキップします。subject="${subject}"`);
            return; // このスレッドのみ安全にスキップ（全体は継続）
          }
          ...
          if (existingSet.has(key)) {
            ...
            // ラベルを付けるかどうかは運用次第だが、二重処理回避のため付与しておく
            thread.markRead();
            thread.addLabel(label);
            return;
          }
          ...
          // 同一実行内の重複も避けるためSetを更新
          existingSet.add(key);

          thread.markRead();
          thread.addLabel(label);
        } catch (inner) {
          console.error("ホワイトペーパー スレッド処理中のエラー:", inner && inner.stack ? inner.stack : inner);
          // このスレッドはスキップし、他スレッド・他セクションは継続
        }
      });
    ...
    console.error("ホワイトペーパーDL セクションでエラー:", e && e.stack ? e.stack : e);
  }
  ...
  try {
    ...
    if (ferretSheet) {
      ...
      ferretThreads.forEach(thread => {
        try {
          ...
          console.error("ferret スレッド処理中のエラー:", inner && inner.stack ? inner.stack : inner);
          // このスレッドはスキップし、他スレッド・他セクションは継続
        }
      });
    ...
    console.error("tool ferret セクションでエラー:", e && e.stack ? e.stack : e);
  }
}
